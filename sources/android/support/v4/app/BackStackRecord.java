...
final class BackStackRecord extends FragmentTransaction implements FragmentManager.BackStackEntry, Runnable {
    ...
    public void run() {
        ...
        if (!this.mAddToBackStack || this.mIndex >= 0) {
            ...
            int i2 = 0;
            TransitionState state = beginTransition(firstOutFragments, lastInFragments, false);
            if (state == null) {
                transitionStyle = this.mTransitionStyle;
            } else {
                transitionStyle = 0;
            }
            if (state == null) {
            ...
            Op op = this.mHead;
            while (op != null) {
                int enterAnim = state != null ? i2 : op.enterAnim;
                int exitAnim = state != null ? i2 : op.exitAnim;
                switch (op.cmd) {
                    case 1:
                    ...
                    case 2:
                        ...
                        if (this.mManager.mAdded != null) {
                            for (int i3 = 0; i3 < this.mManager.mAdded.size(); i3++) {
                                Fragment old = this.mManager.mAdded.get(i3);
                                if (FragmentManagerImpl.DEBUG) {
    ...
    private TransitionState beginTransition(SparseArray<Fragment> firstOutFragments, SparseArray<Fragment> lastInFragments, boolean isBack) {
        ...
        boolean anyTransitionStarted2 = anyTransitionStarted;
        for (int i2 = 0; i2 < lastInFragments.size(); i2++) {
            ...
            if (firstOutFragments.get(containerId) == null && configureTransitions(containerId, state, isBack, firstOutFragments, lastInFragments)) {
                anyTransitionStarted2 = true;
            }
        }
        if (!anyTransitionStarted2) {
            return null;
        }
        return state;
    }
